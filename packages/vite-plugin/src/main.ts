...
export default async function VectorEngine() {
  const editorFolder = path.join(
    path.dirname(
      url.fileURLToPath(await resolve('@vector-engine/editor', import.meta.url))
    ),
    '..'
  )
  ...
  return {
    name: 'vector-engine', // required, will show up in warnings and errors
    load(id: string) {
        ...
        if (req.url === '/') {
          ...
          res.end(
            fs
              ...
              .replace(
                '</body>',
                `<script type="module" src="/@id/__x00__virtual:@vector-engine/test"></script></body>`
              )
          )
          return
