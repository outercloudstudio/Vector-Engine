...
export class Builder {
    ...
    defineAnimatedVectorSetter(property) {
        const me = this;
        return async function* (vector, length, mode) {
            const oldVectorClone = new Vector(me.element[property].x, me.element[property].y, me.element[property].z, me.element[property].w);
            const newVectorClone = new Vector(vector.x, vector.y, vector.z, vector.w);
            for (let i = 1; i <= Math.ceil(length * me.element.scene.engine.frameRate); i++) {
                me.element[property] = oldVectorClone.lerp(newVectorClone, mode(i / Math.ceil(length * me.element.scene.engine.frameRate)));
                yield null;
            }
        };
    }
    defineAnimatedNumberSetter(property) {
        const me = this;
        return async function* (value, length, mode) {
            const oldValue = me.element[property];
            for (let i = 1; i <= Math.ceil(length * me.element.scene.engine.frameRate); i++) {
                me.element[property] = lerp(oldValue, value, mode(i / Math.ceil(length * me.element.scene.engine.frameRate)));
                yield null;
            }
        };
    }
...
export class Link extends TransformBuilder {
    ...
    rotateVector(vector, angle) {
        return new Vector(vector.x * Math.cos(angle) - vector.y * Math.sin(angle), vector.y * Math.cos(angle) + vector.x * Math.sin(angle), vector.z, vector.w);
    }
    updatePosition() {
        for (const link of this.element.links) {
            ...
            const linkRelativePosition = this.element._relativeTransforms[link.id].relativePosition;
            link.position = this.element.position.add(this.rotateVector(linkRelativePosition, angle).multiply(this.element.scale));
    ...
    updateScale() {
        for (const link of this.element.links) {
            if (!(link.builder instanceof TransformBuilder))
                throw new Error('Link can not hold a non transform buffer');
            link.scale = this.element.scale.multiply(this.element._relativeTransforms[link.id].relativeScale);
            this.element._relativeTransforms[link.id].scale = link.scale;
        }
        this.updatePosition();
    }
    updateCache() {
        let updatedRelativeTransform = {};
        for (const link of this.element.links) {
            if (!(link.builder instanceof TransformBuilder))
                throw new Error('Link can not hold a non transform buffer');
            if (this.element._relativeTransforms[link.id] == undefined ||
                !this.element._relativeTransforms[link.id].position.equals(link.position) ||
                !this.element._relativeTransforms[link.id].rotation == link.rotation ||
                ...
                const projectedDeltaPosition = new Vector(deltaMagnitude * Math.cos(angle), deltaMagnitude * Math.sin(angle), deltaPosition.z, deltaPosition.w);
...
export class RenderingBuilder extends TransformBuilder {
    ...
    defineTransition() {
        const me = this;
        return async function* (time, transition, mode) {
            me.element.renderingModifier = transition;
            for (let i = 1; i <= Math.ceil(time * me.element.scene.engine.frameRate); i++) {
                me.element.transitionProgress = mode(i / Math.ceil(time * me.element.scene.engine.frameRate));
                yield null;
            }
    ...
    defineTransitionOut() {
        const me = this;
        return async function* (time, transition) {
            me.element.renderingModifier = transition;
            for (let i = 1; i <= Math.ceil(time * me.element.scene.engine.frameRate); i++) {
...
export class Rect extends RenderingBuilder {
    ...
    bounds() {
        return new Vector(this.element.size.x * this.element.scale.x + this.element.outlineWidth, this.element.size.y * this.element.scale.y + this.element.outlineWidth);
    }
    extent() {
        return new Vector(this.element.size.x * this.element.scale.x, this.element.size.y * this.element.scale.y);
    }
    extentOffset() {
        return new Vector(this.element.outlineWidth / 2, this.element.outlineWidth / 2);
    }
    async render(ctx) {
        ...
        const radiusX = Math.min(Math.max(this.element.radius, 0) * this.element.scale.x, width / 2);
        const radiusY = Math.min(Math.max(this.element.radius, 0) * this.element.scale.y, height / 2);
        ctx.beginPath();
        ctx.moveTo(Math.min(radiusX, width / 2) + offset, offset);
        ctx.lineTo(offset + width - radiusX, offset);
        ctx.quadraticCurveTo(offset + width, offset, offset + width, offset + radiusY);
        ctx.lineTo(offset + width, offset + height - radiusY);
        ctx.quadraticCurveTo(offset + width, offset + height, offset + width - radiusX, offset + height);
        ctx.lineTo(offset + radiusX, offset + height);
        ctx.quadraticCurveTo(offset, offset + height, offset, offset + height - radiusY);
...
export class Ellipse extends RenderingBuilder {
    ...
    async render(ctx) {
        ...
        ctx.lineWidth = this.element.outlineWidth;
        ctx.beginPath();
        ctx.ellipse((this.element.size.x / 2) * this.element.scale.x +
            this.element.outlineWidth / 2, (this.element.size.y / 2) * this.element.scale.y +
            this.element.outlineWidth / 2, (this.element.size.x / 2) * this.element.scale.x, (this.element.size.y / 2) * this.element.scale.y, 0, 0, 2 * Math.PI);
        ctx.fill();
        if (this.element.outlineWidth != 0)
            ctx.stroke();
    }
...
export class Text extends RenderingBuilder {
    ...
    bounds() {
        ...
        return new Vector(measure.width * this.element.scale.x, (measure.actualBoundingBoxDescent + measure.actualBoundingBoxAscent) *
    ...
    async render(ctx) {
        ...
        const outlineAlpha = this.element.outlineColor.w;
        const bounds = this.bounds();
        if (Math.floor(bounds.x) == 0 || Math.floor(bounds.y) == 0)
            return;
        const canvas = new OffscreenCanvas(bounds.x / this.element.scale.x, bounds.y / this.element.scale.y);
        ...
        console.log(measure);
        unscaledCtx.fillText(this.element.text, 0, measure.actualBoundingBoxAscent - 1);
        if (this.element.outlineWidth != 0)
            unscaledCtx.strokeText(this.element.text, 0, measure.actualBoundingBoxAscent - 1);
