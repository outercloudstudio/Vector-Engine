...
impl App {
    ...
    unsafe fn create(window: &Window) -> Result<Self> {
        ...
        create_swapchain(window, &instance, &device, &mut data)?; // Handle swapping images between display and rendering
        create_swapchain_image_views(&device, &mut data)?; // Image view allow us to actually interact with the images
        ...
        create_framebuffers(&device, &mut data)?; // This is what the pipeline will render to
    ...
    unsafe fn render(&mut self, window: &Window) -> Result<()> {
        ...
        let result = self
            .device
            .acquire_next_image_khr(self.data.swapchain, u64::MAX, self.data.image_available_semaphores[self.frame], vk::Fence::null());
    ...
    unsafe fn update_uniform_buffer(&self, image_index: usize) -> Result<()> {
        ...
        let mut proj = cgmath::perspective(Deg(45.0), self.data.swapchain_extent.width as f32 / self.data.swapchain_extent.height as f32, 0.1, 10.0);
        ...
        let memory = self
            .device
            .map_memory(self.data.uniform_buffers_memory[image_index], 0, size_of::<UniformBufferObject>() as u64, vk::MemoryMapFlags::empty())?;
...
unsafe fn check_physical_device_extensions(instance: &Instance, data: &AppData, physical_device: vk::PhysicalDevice) -> Result<()> {
